...
export function CollectionPage({ view }: { view: View }) {
  ...
  const playFirst = (ids: string[]) => { const selectedSongs = ids.map(id => songs.find(item => item.id === id)).filter((song): song is Song => Boolean(song)); if (selectedSongs.length) player.play(selectedSongs[0], selectedSongs); };
  ...
  return <motion.div className="collection-page select-page" initial={{ opacity: 0, x: 22 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -18 }}>
    ...
    <section className="content-section"><div className="section-head"><div><span>YOUR MUSIC</span><h2>{view === 'albums' ? 'Tất cả album' : view === 'artists' ? 'Tất cả nghệ sĩ' : 'Playlist của bạn'}</h2></div><small>{view === 'albums' ? albums.length : view === 'artists' ? artists.length : playlists.length} mục</small></div>
      {view === 'albums' && (albums.length ? <div className="collection-grid">{albums.map(album => <motion.article whileHover={{ y: -7 }} className="collection-card glass" key={album.name}><img src={album.cover} alt="" loading="lazy" decoding="async" /><div><h3>{album.name}</h3><p>{album.artist} · {album.songs.length} bài</p></div><button aria-label="Phát album" onClick={() => playFirst(album.songs.map(song => song.id))}><Play fill="currentColor" /></button></motion.article>)}</div> : <Empty icon={Disc3} text="Chưa có album nào." />)}
      {view === 'artists' && (artists.length ? <div className="collection-grid artists">{artists.map(artist => <motion.article whileHover={{ y: -7 }} className="collection-card glass" key={artist.name}><img src={artist.image} alt="" loading="lazy" decoding="async" /><div><h3>{artist.name}</h3><p>{artist.songs.length} bài hát</p></div><button aria-label="Phát nhạc nghệ sĩ" onClick={() => playFirst(artist.songs.map(song => song.id))}><Play fill="currentColor" /></button></motion.article>)}</div> : <Empty icon={Mic2} text="Chưa có nghệ sĩ nào." />)}
